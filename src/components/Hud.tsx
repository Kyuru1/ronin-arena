...
export default function Hud({ stats, best, onPause, onSelectSlot, onDash, onRaceAbility, raceAbilityBinding, onSpectate, onPotionDismiss, hudScale, language, t, isTouch }: {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-10 flex flex-col justify-between p-2 sm:p-3">
      ...
      <button onClick={onRaceAbility} disabled={!raceAbilityReady} className={`hud-race-ability pointer-events-auto absolute bottom-3 left-3 flex h-[70px] w-[92px] flex-col items-center justify-center gap-1 border-4 bg-[#0b1215] font-pixel shadow-[0_4px_0_#070305] ${stats.raceAbilityT > 0 ? "anim-pulse" : ""}`} style={{ borderColor: race.color, backgroundColor: `${race.color}20` }} title={`${race.ability.name}: ${raceAbilityState}`} aria-label="Habilidade Especial"><strong className="text-center text-[6px]" style={{ color: race.color }}>{race.ability.name}</strong><span className="text-[15px]" style={{ color: race.color }}>{race.icon}</span><span className="text-[7px] text-[#ff6a68]">{raceAbilityState}</span><span className="text-[5px] text-[#d8c2b8]">{raceAbilityBinding}</span></button>
      {!isTouch && <button onClick={activateDash} className={`pointer-events-auto absolute bottom-[84px] left-3 dash-gauge dash-action desktop-dash-button ${stats.dashReady ? "is-ready" : ""}`} title={`${g.dash}: ${stats.dashReady ? t.ready : `${stats.dashCd.toFixed(1)}s`}`} aria-label={g.dash}>
