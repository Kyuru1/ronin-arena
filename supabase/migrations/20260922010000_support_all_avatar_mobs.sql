-- Suporte aos 22 avatares de mobs (AvatarId do cliente) + correcao das
-- validacoes de avatar no banco.
--
-- Antes deste patch o banco so aceitava
--   ('samurai', 'ninja', 'oni', 'boss', 'bat')
-- em public.profiles.avatar_id, public.ranking.avatar_id e nas funcoes
-- submit_ranking / sync_ranking_avatar. O cliente ja envia os 22 mobs, entao
-- trocar de avatar ou salvar ranking com um mob novo rejeitava no servidor.

-- ---------------------------------------------------------------------------
-- 0) Seguranca: tabela de rate-limit usada por submit_ranking (idempotente).
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.ranking_submission_limits (
  user_id UUID PRIMARY KEY REFERENCES auth.users (id) ON DELETE CASCADE,
  last_submit_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

ALTER TABLE public.ranking_submission_limits ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.ranking_submission_limits FROM PUBLIC, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 1) Normaliza qualquer avatar legado/invalido antes de apertar os CHECKs.
-- ---------------------------------------------------------------------------
UPDATE public.profiles
SET avatar_id = 'samurai'
WHERE avatar_id IS NULL
   OR avatar_id NOT IN (
    'samurai', 'grunt', 'bat', 'brute', 'spitter', 'ninja', 'hound', 'wisp',
    'archer', 'oni', 'shield', 'slime', 'monk', 'demon', 'skeleton', 'crawler',
    'bomber', 'bombMinion', 'ram', 'warlock', 'golem', 'boss'
  );
...
-- ---------------------------------------------------------------------------
-- 2) Substitui os CHECKs de avatar por uma lista com os 22 mobs.
--    (Remove qualquer constraint de check que fale em avatar_id, com qualquer
--    nome gerado automaticamente, e cria as novas com nome conhecido.)
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  c RECORD;
BEGIN
  ...
  FOR c IN
    ...
    WHERE conrelid = 'public.ranking'::regclass
      ...
      AND pg_get_constraintdef(oid) ILIKE '%avatar_id%'
  LOOP
    EXECUTE format('ALTER TABLE public.ranking DROP CONSTRAINT %I', c.conname);
  END LOOP;
END $$;
...
ALTER TABLE public.ranking
  ADD CONSTRAINT ranking_avatar_id_check
  CHECK (avatar_id IN (
    'samurai', 'grunt', 'bat', 'brute', 'spitter', 'ninja', 'hound', 'wisp',
    'archer', 'oni', 'shield', 'slime', 'monk', 'demon', 'skeleton', 'crawler',
    'bomber', 'bombMinion', 'ram', 'warlock', 'golem', 'boss'
...
-- ---------------------------------------------------------------------------
-- 3) submit_ranking: aceita os 22 avatares (mantem validacao, rate-limit de
--    30s e "so substitui se o score for maior").
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF p_score IS NULL OR p_score NOT BETWEEN 0 AND 10000000
    ...
    OR p_survival_time_seconds IS NULL OR p_survival_time_seconds NOT BETWEEN 0 AND 86400 THEN
  ...
  IF char_length(final_player_name) NOT BETWEEN 1 AND 12
    OR final_player_name !~ '^[A-Z0-9 _-]+$' THEN
  ...
  INSERT INTO public.ranking (
    ...
    current_user_id, final_player_name, final_avatar_id, p_difficulty,
    p_score, p_wave, p_kills, p_survival_time_seconds
  )
  ...
  IF saved.id IS NULL THEN
    SELECT * INTO saved FROM public.ranking
...
-- ---------------------------------------------------------------------------
-- 4) sync_ranking_avatar: aceita os 22 avatares.
-- ---------------------------------------------------------------------------
