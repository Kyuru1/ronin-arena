...
function weaponDescription(w: Weapon, t: Strings, g: GameplayStrings) { if (w === "harp") return "Cura e chicoteia em três direções."; if (w === "godslayer") return "Lâmina divina de alto dano e grande alcance."; return w === "book" ? g.bookDesc : w === "staff" ? g.staffDesc : t[`${w}Desc` as keyof Strings] as string; }
...
export default function ShopScreen({ wave, stats, onBuyWeapon, onSellWeapon, onSelectSlot, onReorderWeapons, onBuyPowerUp, onUpgradeWeapon, onMagicType, onCloseShop, language, t, abilityBinding }: {
  ...
  const g = GAMEPLAY_TEXT[language]; const maxWeapons = stats.maxWeaponSlots; const info = ALL_WEAPONS.find((item) => item.id === selected) ?? ALL_WEAPONS[0]; const selectedIndex = stats.weapons.indexOf(selected); const owned = selectedIndex >= 0; const levels = stats.weaponLevels[selected];
  ...
  return <div className="px-backdrop absolute inset-0 z-30 flex items-center justify-center overflow-y-auto p-2 sm:p-5"><PxFrame title={t.waveClearedTitle.replace("{wave}", String(wave))} icon="icoTrophy" className="shop-frame anim-pop relative my-auto flex max-h-[96vh] w-full max-w-5xl flex-col overflow-hidden p-3 pt-10 sm:p-5 sm:pt-11">
    ...
    {tab === "menu" && <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto pr-1">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <button className="shop-category" onClick={() => setTab("weapons")}><strong>ARMAS</strong><span>Compre, venda e organize seus slots.</span></button>
        ...
        <button className="shop-category" onClick={() => setTab("character")}><strong>PERSONAGEM</strong><span>Abra a ficha completa da run.</span></button>
      </div>
    </div>}
