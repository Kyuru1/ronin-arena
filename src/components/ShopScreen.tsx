...
import { localizedWeapon, SHOP_UI } from "../game/localizedContent";
...
export default function ShopScreen({ wave, stats, onBuyWeapon, onSellWeapon, onSelectSlot, onReorderWeapons, onBuyPowerUp, onUpgradeWeapon, onMagicType, onCloseShop, language, t, abilityBinding }: {
  wave: number; stats: HudStats; onBuyWeapon: (w: Weapon, cost: number) => void; onSellWeapon: (slot: number, refund: number) => void; onSelectSlot: (slot: number) => void; onReorderWeapons: (from: number, to: number) => void; onBuyPowerUp: (p: PowerUp, cost: number) => void; onUpgradeWeapon: (w: Weapon, upgrade: WeaponUpgrade, cost: number) => void; onMagicType: (type: MagicType) => void; onCloseShop: () => void; language: Language; t: Strings; abilityBinding: string;
}) {
  useMenuNavigation();
  ...
  const g = GAMEPLAY_TEXT[language]; const ui = SHOP_UI[language]; const maxWeapons = stats.maxWeaponSlots; const info = ALL_WEAPONS.find((item) => item.id === selected) ?? ALL_WEAPONS[0]; const selectedIndex = stats.weapons.indexOf(selected); const owned = selectedIndex >= 0; const levels = stats.weaponLevels[selected];
  ...
  return <div className="px-backdrop absolute inset-0 z-30 flex items-center justify-center overflow-y-auto p-2 sm:p-5"><PxFrame title={t.waveClearedTitle.replace("{wave}", String(wave))} icon="icoTrophy" className="shop-frame anim-pop relative my-auto flex max-h-[96vh] w-full max-w-5xl flex-col overflow-hidden p-3 pt-10 sm:p-5 sm:pt-11">
    <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2">{tab === "menu" ? <strong className="font-pixel text-[8px] text-[#ffd44a]">{ui.choose}</strong> : <button className="shop-tab" onClick={() => setTab("menu")}>◀ {ui.back}</button>}</div><CoinBox coins={stats.coins} /></div><div className="px-divider my-3" />
    {tab === "menu" && <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto pr-1">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <button className="shop-category" onClick={() => setTab("weapons")}><strong>{ui.weapons}</strong><span>{ui.weaponsDesc}</span></button>
        <button className="shop-category" onClick={() => setTab("stats")}><strong>{ui.stats}</strong><span>{ui.statsDesc}</span></button>
        <button className="shop-category" onClick={() => setTab("upgrades")}><strong>{ui.upgrades}</strong><span>{ui.upgradesDesc}</span></button>
        <button className="shop-category" onClick={() => setTab("character")}><strong>{ui.character}</strong><span>{ui.characterDesc}</span></button>
      </div>
    </div>}
    {tab === "weapons" && <div className="shop-layout scrollbar-thin grid min-h-0 flex-1 gap-3 overflow-y-auto lg:grid-cols-[1fr_1.2fr]"><div className="flex min-w-0 flex-col gap-2"><div className="font-pixel text-[7px] text-[#ffd44a]">{t.weaponsTitle}</div><div className="grid gap-1 sm:grid-cols-2">{ALL_WEAPONS.map((item) => renderWeapon(item.id))}</div><div className="px-inset p-3"><div className="mb-2 font-pixel text-[7px] text-[#ffd44a]">{ui.slots}</div><div className="grid grid-cols-4 gap-1 sm:grid-cols-6">{Array.from({ length: maxWeapons }).map((_, index) => { const weapon = stats.weapons[index]; return weapon ? <button key={weapon} draggable onDragStart={() => setDragged(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragged !== null) onReorderWeapons(dragged, index); setDragged(null); }} onDragEnd={() => setDragged(null)} onClick={() => { onSelectSlot(index); select(weapon); }} className={`px-tile flex h-16 min-w-0 cursor-grab flex-col items-center justify-center gap-1 p-1 ${index === stats.activeSlot ? "outline outline-2 outline-[#ffd44a]" : ""}`}><WeaponPreview weapon={weapon} form={stats.weaponLevels[weapon].form} scale={1} /><span className="font-pixel text-[6px]">{index + 1}</span></button> : <div key={index} className="px-empty flex h-16 items-center justify-center font-pixel text-[7px] text-[#6c3a42]">{index + 1}</div>; })}</div></div></div><div className="flex min-w-0 flex-col gap-2"><div className="px-inset relative flex min-h-40 items-center justify-center overflow-hidden"><WeaponPreview weapon={selected} scale={3} angle={-30} form={levels.form} /><span className="font-pixel absolute left-3 top-3 text-[8px] text-[#ffd44a]">{weaponName(selected, t, g, language, levels.form)}</span></div><div className="px-inset p-3"><p className="font-pixel text-[7px] leading-5 text-[#d8a9a0]">{weaponDescription(selected, t, g, language)}</p><div className="mt-3 grid grid-cols-3 gap-2 text-[6px] text-[#a35662]"><span>{t.dmg}<Pips value={Math.min(5, info.dmg + levels.damage)} color="#e0444d" /></span><span>{t.spd}<Pips value={Math.min(5, info.spd + levels.speed)} color="#ffd44a" /></span><span>{t.range}<Pips value={Math.min(5, info.range + levels.range)} color="#62b7c8" /></span></div></div>{owned ? <PxButton tone="dark" disabled={stats.weapons.length <= 1} onClick={() => onSellWeapon(selectedIndex, info.sellRefund)} className="w-full py-3 text-[7px]">{t.sell} <Price cost={info.sellRefund} /></PxButton> : <PxButton tone="gold" disabled={stats.coins < info.cost || stats.weapons.length >= maxWeapons} onClick={() => onBuyWeapon(selected, info.cost)} className="w-full py-3 text-[8px]">{stats.weapons.length >= maxWeapons ? t.maxSlots : <>{t.buy} <Price cost={info.cost} /></>}</PxButton>}</div></div>}
    {tab === "upgrades" && <div className="shop-layout scrollbar-thin grid min-h-0 flex-1 gap-3 overflow-y-auto lg:grid-cols-[220px_1fr]"><div className="flex min-w-0 flex-col gap-2"><div className="font-pixel text-[7px] text-[#ffd44a]">{ui.owned}</div>{stats.weapons.map(renderWeapon)}</div><div className="flex min-w-0 flex-col gap-2"><div className="px-inset flex items-center gap-3 p-3"><WeaponPreview weapon={selected} form={levels.form} scale={2} /><div><strong className="font-pixel text-[8px] text-[#ffd44a]">{weaponName(selected, t, g, language, levels.form)}</strong><p className="mt-2 font-pixel text-[6px] leading-4 text-[#a9c3be]">{weaponDescription(selected, t, g, language)}</p></div></div>{UPGRADE_TYPES.map(renderUpgrade)}{selected === "book" && <div className="px-inset p-3"><div className="mb-2 font-pixel text-[7px] text-[#ffd44a]">{g.magic}</div><div className="grid grid-cols-2 gap-1">{MAGIC_TYPES.map((magic) => <button key={magic} onClick={() => onMagicType(magic)} className={`magic-option ${stats.magicType === magic ? "is-active" : ""}`}><strong>{g[magic]}</strong><span>{g[`${magic}Desc` as keyof typeof g]}</span></button>)}</div></div>}</div></div>}
        {tab === "stats" && <div className="scrollbar-thin grid min-h-0 flex-1 gap-3 overflow-y-auto md:grid-cols-[.8fr_1.2fr]"><div className="px-inset p-4"><div className="font-pixel mb-3 text-[9px] text-[#ffd44a]">{t.yourStats}</div><div className="grid grid-cols-2 gap-2 font-pixel text-[7px]"><div className="stat-readout"><span>{t.statHearts}</span><strong>{stats.hp}/{stats.maxHp}</strong></div><div className="stat-readout"><span>{t.statSpeed}</span><strong>{108 + stats.speedBonus}</strong></div><div className="stat-readout"><span>{t.statDashCd}</span><strong>{stats.dashMax.toFixed(1)}s</strong></div><div className="stat-readout"><span>{t.statDashDist}</span><strong>x{stats.dashSpeedMult.toFixed(2)}</strong></div></div></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-2">{POWERUPS.map((power) => { const maxed = isPowerUpAtLimit(stats, power.id, stats.difficulty); return <div key={power.id} className="px-inset flex flex-col p-3"><div className="flex items-center gap-2"><PixelSprite name={power.icon} scale={2} /><strong className="font-pixel text-[7px]">{t[power.nameKey] as string}</strong></div><p className="font-pixel my-2 grow text-[6px] leading-4 text-[#a35662]">{t[power.descKey] as string}</p><PxButton tone="red" disabled={maxed || stats.coins < power.cost} onClick={() => onBuyPowerUp(power.id, power.cost)} className="py-2 text-[7px]">{maxed ? g.max : <>{g.improve} <Price cost={power.cost} /></>}</PxButton></div>; })}</div></div>}    {tab === "character" && <div className="shop-stats-panel scrollbar-thin min-h-0 flex-1 overflow-y-auto">
      <CharacterDetails data={{
        ...
        kills: stats.kills, score: stats.score, time: stats.time, perkBuffT: stats.perkBuffT,
        perkEchoReady: stats.perkEchoReady,
