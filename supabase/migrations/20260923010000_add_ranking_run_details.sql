-- Snapshot detalhado da melhor run de cada jogador/dificuldade.
...
ALTER TABLE public.ranking ADD CONSTRAINT ranking_race_id_check
  CHECK (race_id IS NULL OR race_id IN ('godHunter', 'oniBlood', 'spiritualHeir', 'ronin', 'survivor'));
...
ALTER TABLE public.ranking ADD CONSTRAINT ranking_weapons_check
  CHECK (
    cardinality(weapons) <= 7
    AND weapons <@ ARRAY['katana', 'bow', 'hammer', 'shield', 'mine', 'book', 'staff']::TEXT[]
  );
...
BEGIN
  ...
  IF p_race_id IS NOT NULL AND p_race_id NOT IN ('godHunter', 'oniBlood', 'spiritualHeir', 'ronin', 'survivor') THEN
  ...
  IF cardinality(safe_weapons) > 7
    OR NOT (safe_weapons <@ ARRAY['katana', 'bow', 'hammer', 'shield', 'mine', 'book', 'staff']::TEXT[])
  THEN
