...
ALTER TABLE public.ranking ADD CONSTRAINT ranking_weapons_check
  CHECK (
    cardinality(weapons) <= 12
    AND weapons <@ ARRAY['katana', 'bow', 'hammer', 'shield', 'mine', 'book', 'staff', 'harp', 'godslayer', 'boomerang', 'shuriken', 'spear']::TEXT[]
  );
...
BEGIN
  ...
  IF cardinality(safe_weapons) > 12
    OR NOT (safe_weapons <@ ARRAY['katana', 'bow', 'hammer', 'shield', 'mine', 'book', 'staff', 'harp', 'godslayer', 'boomerang', 'shuriken', 'spear']::TEXT[])
  THEN
